...
(defn- check-valid [e validation-fn error-handler data-path err-msg]
  (try
    (cond (and (fn? validation-fn) (not (validation-fn e)))
          ...
          e)
    (catch Exception ex
      (error-handler (str err-msg " (" ex ")") data-path e))))
...
(defn is-string
  ...
  ([validation-fn err-msg]
     (fn check-is-string [e data-path error-handler metadata-handler]
       (if (string? e)
...
(defn is-keyword
  ([] (is-keyword nil nil))
  ([validation-fn err-msg]
     (fn check-is-keyword [e data-path error-handler metadata-handler]
       (if (keyword? e)
         (check-valid e validation-fn error-handler data-path err-msg)
         (error-handler "should be a keyword" data-path e)))))

(defn is-boolean []
  (fn check-is-boolean [e data-path error-handler metadata-handler]
...
(defn is-decimal
  ([] (is-decimal nil nil))
  ([validation-fn err-msg]
     (fn check-is-decimal [e data-path error-handler metadata-handler]
...
(defn is-date []
  (let [parse-fmts [(DateTimeFormat/forPattern "yyyy-MM-dd'T'HH:mm:ss.SSSZ") (DateTimeFormat/forPattern "EEE, dd MMM yyyy HH:mm:ss zzz")]]
    (fn check-is-date [e data-path error-handler metadata-handler]
...
(defn should-exist []
  (fn check-should-exist [data data-path error-handler metadata-handler]
    (if (nil? data)
      (error-handler "should exist" data-path data)
      data)))

(defn is-nil []
  (fn check-is-nil [data data-path error-handler metadata-handler]
    (when-not (nil? data)
...
(defn equals
  ...
  ([v error-message]
     (fn check-equals [data data-path error-handler metadata-handler]
       (if (not= v data)
...
(defn coerce [error-message coerce-func type-def]
  (with-meta
    (fn running-coerce [data data-path error-handler metadata-handler]
      (try
...
(defn map-of [& keys-and-vals]
  (with-meta
    (let [keys-and-vals (apply hash-map keys-and-vals)]
      (fn check-map-of [data data-path error-handler metadata-handler]
        (if (map? data)
          (reduce (fn running-map-of-check [m [k val-type-def]]
...
(defn map-of-values [key-func val-type-def]
  (fn check-map-of-values [data data-path error-handler metadata-handler]
    (if (map? data)
      (->> data vals
...
(defn list-of [type-def]
  (fn check-list-of [data data-path error-handler metadata-handler]
    (if (sequential? data)
      (->> data
...
(defn one-of [& type-defs]
  (fn check-one-of [data data-path error-handler metadata-handler]
...
(defn all-of [& type-defs]
  (fn check-all-of [data data-path error-handler metadata-handler]
    (->> type-defs
...
(defn count-is [message func & args]
  (fn check-count-is [data data-path error-handler metadata-handler]
...
(defn with-defaults [type-def & {:as defaults}]
  (with-meta (fn running-with-defaults [data data-path error-handler metadata-handler]
...
(defn with-modifications [type-def & modifier-fns]
  (with-meta
    (fn running-with-modifications [data data-path error-handler metadata-handler]
      (try
...
(defn compose-with [merge-fn & type-defs]
  (with-meta
    (fn running-compose-with [data data-path error-handler metadata-handler]
      (->> type-defs
        (map #(validate-element % data data-path error-handler metadata-handler))
        (apply merge-fn)))
    (->> type-defs
